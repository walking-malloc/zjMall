...
func GenerateJWT(userID string, expiresIn time.Duration) (string, int64, error) {
	return GenerateJWTWithRoles(userID, nil, expiresIn)
}

// GenerateJWTWithRoles 生成带角色的 JWT Token
// userID: 用户 ID
// roles: 用户角色列表
// expiresIn: Token 过期时长
// 返回: token 字符串、过期时间戳、错误
func GenerateJWTWithRoles(userID string, roles []string, expiresIn time.Duration) (string, int64, error) {
	// 计算过期时间
...
func GenerateJWTWithRememberMe(userID string, rememberMe bool) (string, int64, error) {
	return GenerateJWTWithRolesAndRememberMe(userID, nil, rememberMe)
}

// GenerateJWTWithRolesAndRememberMe 根据 RememberMe 生成带角色的 Token（便捷函数）
// userID: 用户 ID
// roles: 用户角色列表
// rememberMe: 是否记住我
// 返回: token 字符串、过期时间戳、错误
func GenerateJWTWithRolesAndRememberMe(userID string, roles []string, rememberMe bool) (string, int64, error) {
	...
	return GenerateJWTWithRoles(userID, roles, expiresIn)
}
...
func VerifyJWT(tokenString string) (string, error) {
	claims, err := VerifyJWTWithClaims(tokenString)
	if err != nil {
		return "", err
	}
...
// VerifyJWTWithClaims 验证 JWT Token 并返回 Claims
// tokenString: JWT Token 字符串
// 返回: Claims 和错误
func VerifyJWTWithClaims(tokenString string) (*Claims, error) {
	// 解析 Token
